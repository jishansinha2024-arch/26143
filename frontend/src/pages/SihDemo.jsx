...
const Row = ({ k, v, testid }) => <div className="flex justify-between gap-3 border-b border-dashed py-1 font-mono text-[11px]" style={{ borderColor: "var(--border-default)" }}><span className="text-slate-500">{k}</span><span className="text-right text-slate-800" data-testid={testid}>{v ?? "UNAVAILABLE"}</span></div>;
const Tag = ({ children, tone = "#1F7F93", testid }) => <span data-testid={testid} className="rounded px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-wider" style={{ color: tone, border: `1px solid ${tone}66` }}>{children}</span>;
...
export default function SihDemo() {
  ...
  const steps = d ? [
    { t: "Problem", icon: Globe2, body: <>
      <p className="text-sm text-slate-700">Oil discharges at sea are rarely witnessed. Varuna Netra combines <b>real Sentinel-1 SAR acquisitions</b>, <b>real AIS vessel telemetry</b> and explainable space-time correlation so an operator can rank <i>candidate</i> vessels and open an evidence-backed case — anywhere in the world.</p>
      ...
      <p className="mt-3 font-mono text-[10px] text-amber-700" data-testid="demo-reference-label">REFERENCE CASE — STORED DATA · {d.c.case_number} · {d.c.origin_label} · pinned by {d.ref.pinned_by}. Nothing on this walkthrough is simulated; values are read from the database and provider catalogues.</p></> },
    ...
    { t: "Sentinel-1 scene", icon: Satellite, body: <>
      <div className="mb-2 flex gap-2"><Tag tone="#2E8B6A" testid="demo-scene-badge">{d.c.scene_status?.state === "SAR_READY" ? "REAL SENTINEL-1 · SAR READY" : d.c.scene_status?.state}</Tag><Tag>LATEST AVAILABLE ACQUISITION — not real-time</Tag></div>
    ...
    { t: "Spill candidate", icon: Waves, body: <>
      <div className="mb-2 flex gap-2"><Tag tone="#C48A22">EXPERIMENTAL SAR DARK-SPOT DETECTOR</Tag><Tag tone="#7D919C">validation pending</Tag></div>
      ...
      <p className="mt-2 text-[11px] text-amber-700/90">Dark formations in SAR imagery may also be caused by low wind, biogenic films or other oceanographic phenomena. Detection is an investigation candidate, not automatic proof of an oil spill.</p></> },
    { t: "AIS vessels", icon: Radio, body: <>
      <div className="mb-2 flex gap-2"><Tag tone={d.ais.state === "LIVE" ? "#2E8B6A" : "#C48A22"} testid="demo-ais-state">AISStream now: {d.ais.feed || d.ais.state}</Tag></div>
    ...
    { t: "Candidate ranking · Why this vessel?", icon: Ship, body: d.cands.length ? <>
      ...
      <div className="rounded border" style={{ borderColor: "var(--border-default)" }}><CandidatesTable candidates={d.cands} selected={selected} onSelect={setSelected} /></div></> : <p className="text-xs text-amber-700" data-testid="demo-no-candidates">No AIS candidate vessels were found in this case's space-time corridor — the system reports this truthfully rather than inventing a vessel. Correlation can be re-run once AIS history covers the window.</p> },
    ...
    { t: "Investigation case", icon: FolderOpen, body: <>
      ...
      <button data-testid="demo-open-case" onClick={() => nav(`/cases/${d.c.id}`)} className="mt-3 inline-flex items-center gap-1 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper"><FolderOpen size={12} /> Open the full case (map, tracks, PDF evidence)</button></> },
  ] : [];
  ...
  return (
    <div className="h-full overflow-y-auto p-6" data-testid="sih-demo-page">
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <Tag tone="#C48A22" testid="demo-mode-tag">REFERENCE CASE — STORED DATA · not LIVE</Tag>
        {hasRole(user, "admin") && <button data-testid="demo-unpin" onClick={async () => { if (!window.confirm("Unpin the SIH reference case? Run SIH Demo will show REFERENCE CASE NOT PINNED until an admin pins another case.")) return; try { await api.delete("/demo/reference"); toast.success("Reference case unpinned"); load(); } catch (e) { toast.error(apiError(e)); } }} className="ml-auto inline-flex items-center gap-1 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-amber-700" style={{ borderColor: "var(--border-highlight)" }} title="Admin: unpin the reference case (pin another from any case's detail page)"><PinOff size={12} /> Unpin reference</button>}
        <button data-testid="demo-restart" onClick={() => { setStep(0); load(); }} className={`${hasRole(user, "admin") ? "" : "ml-auto "}inline-flex items-center gap-1 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-600`} style={{ borderColor: "var(--border-highlight)" }}><Play size={12} /> Restart</button>
      </div>
      {err && <p className="rounded border border-rose-500/50 px-3 py-2 text-xs text-rose-600" data-testid="demo-error">Demo unavailable: {err}</p>}
      ...
      {d && (
        <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
          <ol className="space-y-1" data-testid="demo-steps">
            {steps.map((s, i) => <li key={s.t}><button data-testid={`demo-step-${i}`} onClick={() => setStep(i)} className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-mono text-[11px] ${i === step ? "bg-tide/15 text-tide" : "text-slate-400 hover:text-slate-800"}`}><s.icon size={12} /> {i + 1}. {s.t}</button></li>)}
          </ol>
          <div className="panel p-5 fade-up" key={step} data-testid="demo-step-panel">
            ...
            <div className="mt-5 flex justify-between">
              <button data-testid="demo-prev" disabled={step === 0} onClick={() => setStep(step - 1)} className="inline-flex items-center gap-1 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-600 disabled:opacity-40" style={{ borderColor: "var(--border-highlight)" }}><ChevronLeft size={12} /> Back</button>
              <button data-testid="demo-next" disabled={step === steps.length - 1} onClick={() => setStep(step + 1)} className="inline-flex items-center gap-1 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper disabled:opacity-40">Next <ChevronRight size={12} /></button>
            </div>
          </div>
        </div>
      )}
