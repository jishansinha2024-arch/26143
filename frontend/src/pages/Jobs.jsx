          ...
          {shown.map((a) => (
            <div key={a.id} className="rounded border p-3 text-xs" style={{ borderColor: a.acknowledged ? "var(--border-default)" : "rgba(186,26,26,0.5)", background: a.acknowledged ? "transparent" : "rgba(186,26,26,0.05)" }} data-testid={`jobs-alert-${a.id}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-1.5"><ShieldAlert size={12} color={a.severity === "high" ? "#ba1a1a" : a.severity === "medium" ? "#b26a00" : "#5b86b3"} /><span className="font-mono text-[10px] uppercase tracking-wider text-slate-400">{a.kind?.replace("_", " ")}</span></span>
              ...
              {a.notification && <p className="mt-1 font-mono text-[10px]" data-testid={`alert-notification-${a.id}`} style={{ color: a.notification.status === "sent" ? "#006a61" : a.notification.status === "not_configured" ? "#b26a00" : "#5b86b3" }}>email: {a.notification.status.replace("_", " ")} · {a.notification.sent}/{a.notification.recipients?.length || 0} recipients</p>}
