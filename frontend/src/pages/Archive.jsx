  ...
  <div className="panel overflow-hidden" style={{ height: "70vh" }} data-testid="archive-map">
    {rows.length === 0 ? (
      ...
      <MapContainer center={[12, 74]} zoom={4} worldCopyJump style={{ height: "100%", width: "100%", background: "#0b1220" }}>
        ...
        {rows.map((r) => (
          <CircleMarker key={r.id} center={[r.lat, r.lon]} radius={7} pathOptions={{ color: "#1F7F93", fillColor: "#1F7F93", fillOpacity: 0.6, weight: 1.5 }}>
            <Popup>
    ...
    <div className="h-full overflow-y-auto p-6" data-testid="archive-page">
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <div className="flex overflow-hidden rounded border" style={bd} data-testid="archive-view-toggle">
          <button data-testid="archive-view-list" onClick={() => setView("list")} className={`flex items-center gap-1.5 px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider ${view === "list" ? "bg-ink text-paper" : "text-slate-600 hover:bg-paper"}`}><List size={12} /> List</button>
          <button data-testid="archive-view-map" onClick={() => setView("map")} className={`flex items-center gap-1.5 px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider ${view === "map" ? "bg-ink text-paper" : "text-slate-600 hover:bg-paper"}`}><MapPin size={12} /> Map</button>
        </div>
        {hasRole(user, "supervisor") && <button data-testid="btn-archive-add" onClick={() => setShowForm(!showForm)} className="inline-flex items-center gap-1.5 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper"><Plus size={12} /> Add incident</button>}
      </div>
      <ArchiveLessons />
      {showForm && (
        <div className="panel mb-4 grid grid-cols-4 gap-2 p-4" data-testid="archive-form">
          ...
          <button data-testid="archive-submit" onClick={create} disabled={f.name.length < 3 || !f.date || f.lat === "" || f.lon === ""} className="col-span-4 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper disabled:opacity-50">Save incident</button>
        </div>
      )}
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3" data-testid="archive-list">
        {rows.map((r) => (
          <article key={r.id} data-testid={`archive-entry-${r.id}`} className="panel p-4 text-xs">
            <div className="flex items-start gap-2"><BookOpen size={13} color="#1F7F93" className="mt-0.5 shrink-0" /><h2 className="font-display text-base font-semibold text-slate-800">{r.name}</h2><Link to={`/archive/${r.id}`} data-testid={`archive-open-vault-${r.id}`} className="ml-auto shrink-0 rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-tide hover:bg-tide/10" style={bd}>Open case file</Link></div>
            ...
            <p className="mt-2 text-slate-600"><span className="text-slate-500">Cause:</span> {r.cause}</p>
            <p className="text-slate-600"><span className="text-slate-500">Source:</span> {r.vessel_facility}</p>
            <p className="mt-1 text-slate-400"><span className="text-slate-500">Ecosystems:</span> {r.ecosystems.join(", ")}</p>
            <p className="mt-1 text-slate-600"><span className="text-slate-500">Remediation:</span> {r.remediation.join("; ")}</p>
            <p className="mt-2 italic text-tide/80">{r.lessons}</p>
          </article>
        ))}
      </div>
      </>)}
