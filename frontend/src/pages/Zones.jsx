    ...
    <div className="flex h-full overflow-hidden" data-testid="zones-page">
      <div className="relative flex-1">
        <MapContainer center={[15, 60]} zoom={3} className="h-full w-full" worldCopyJump doubleClickZoom={!drawing}>
          ...
          {aoi?.geometry && <GeoJSON key={`aoi-${aoi.updated_at}`} data={aoi.geometry} style={{ color: "#2A93A8", weight: 2, dashArray: "10,5", fillOpacity: 0.05 }} />}
      ...
      <aside className="flex w-[480px] shrink-0 flex-col overflow-y-auto border-l p-4" style={{ borderColor: "var(--border-default)", background: "var(--bg-secondary)" }}>
        ...
        {selected && (
          <div className="mt-4 rounded border p-3 text-xs" style={{ borderColor: "var(--border-highlight)" }} data-testid={`zone-detail-${selected.code}`}>
            <div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-sm" style={{ background: TYPE_COLOR[selected.zone_type] }} /><span className="font-mono text-tide">{selected.code}</span><ProvBadge p={selected.provenance} testId="zone-detail-provenance" />
              {admin && <span className="ml-auto flex items-center gap-1"><button data-testid={`zone-toggle-${selected.code}`} onClick={() => toggle(selected)} className="rounded px-1.5 py-0.5 font-mono text-[10px] text-slate-400 hover:text-amber-700">{selected.active === false ? "enable" : "disable"}</button><button data-testid={`zone-delete-${selected.code}`} onClick={() => remove(selected)} className="rounded p-1 text-slate-400 hover:text-rose-600"><Trash2 size={12} /></button></span>}</div>
            <div className="mt-1 text-slate-800">{selected.name}</div>
            ...
            <div className="mt-1 text-slate-400">{selected.authority}{selected.authority_verified === false && <span className="ml-1 text-amber-700">(reference name — verify before contact)</span>}</div>
            ...
            {hasRole(user, "analyst") && <button data-testid="zone-detail-use-aoi" onClick={async () => { try { const { data } = await api.post("/aoi/select", { kind: "zone", zone_id: selected.id }); setAoi(data); setFlyTo(data.bbox); toast.success(`AOI set: ${data.name}`); } catch (e) { toast.error(apiError(e)); } }} className="mt-2 rounded bg-ink px-2 py-1 font-mono text-[10px] font-semibold uppercase text-paper">use this zone as investigation AOI</button>}
          </div>
        )}
        <ZoneRules zones={cat?.zones || []} />
        <IcgDistricts onChanged={loadIcg} />
        {admin && (
          <div className="mt-5 rounded border p-4" style={{ borderColor: "rgba(31,127,147,0.35)", background: "rgba(31,127,147,0.04)" }} data-testid="zone-import-form">
            <div className="mb-2 flex items-center gap-2"><Globe size={14} color="#2A93A8" /><h2 className="font-display font-semibold">Import reference boundaries</h2></div>
            <p className="mb-2 text-[11px] text-slate-400">Marine Regions Maritime Boundaries v12 (CC-BY 4.0) via WFS — validated, repaired, simplified, stored with a 2dsphere index. Use <span className="font-mono text-tide">ALL</span> for the whole world or ISO3 codes.</p>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 font-mono text-[10px]" data-testid="zone-import-layers">
              {[["eez", "EEZ 200 NM"], ["eez_24nm", "Contiguous 24 NM"], ["eez_12nm", "Territorial 12 NM"]].map(([k, l]) => (
                <label key={k} className="flex items-center gap-1 text-slate-600"><input type="checkbox" data-testid={`zone-import-layer-${k}`} checked={importLayers.includes(k)} onChange={(e) => setImportLayers(e.target.checked ? [...importLayers, k] : importLayers.filter((x) => x !== k))} /> {l}</label>))}
              <button data-testid="zone-import-india-preset" onClick={() => { setIso("IND"); setImportLayers(["eez", "eez_24nm", "eez_12nm"]); }} className="rounded border px-2 py-0.5 uppercase tracking-wider text-amber-700" style={{ borderColor: "rgba(184,134,42,0.5)" }}>India · all 3 zones</button>
            </div>
            <div className="mt-2 flex items-center gap-2">
              <button data-testid="btn-import-eez" disabled={!!importing} onClick={importZones} className="inline-flex items-center gap-1.5 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper hover:bg-tide disabled:opacity-50"><Globe size={12} /> {importing ? "Importing…" : "Import from Marine Regions"}</button>
              {importing && <span className="truncate font-mono text-[10px] text-tide" data-testid="zone-import-status">{importing}</span>}
            </div>
          </div>
        )}
        {admin && (
          <div className="mt-5 rounded border p-4" style={{ borderColor: "var(--border-default)" }} data-testid="zone-create-form">
            <div className="mb-3 flex items-center gap-2"><MapIcon size={14} color="#2A93A8" /><h2 className="font-display font-semibold">Add USER-DEFINED zone</h2></div>
