    ...
    <div className="flex h-full overflow-hidden" data-testid="case-detail">
      <div className="relative flex-1">
        ...
        <div className="absolute left-3 top-3 z-[1000] flex items-center gap-2">
          ...
          <button data-testid="map-toggle-ais-layer" onClick={() => setShowTracks(!showTracks)} className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider" style={{ ...overlayBtn, color: showTracks ? "#0a67ad" : "#5b86b3" }}><Layers size={12} /> AIS tracks</button>
          <button data-testid="map-toggle-zones-layer" onClick={() => setShowZones(!showZones)} className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider" style={{ ...overlayBtn, color: showZones ? "#0a67ad" : "#5b86b3" }}><Layers size={12} /> Zones</button>
          {showZones && (
            <span className="inline-flex items-center gap-1 rounded px-1.5 py-1" style={overlayBtn} data-testid="zone-kind-toggles">
              {[["territorial", "12 NM"], ["contiguous", "24 NM"], ["eez", "EEZ"]].map(([k, l]) => (
                <button key={k} data-testid={`zone-kind-${k}`} onClick={() => setZoneKinds({ ...zoneKinds, [k]: !zoneKinds[k] })} className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: zoneKinds[k] ? ZONE_STYLE[k].color : "#5b86b3", background: zoneKinds[k] ? `${ZONE_STYLE[k].color}22` : "transparent" }}>
                  <span className="inline-block h-0 w-3 border-t-2" style={{ borderColor: zoneKinds[k] ? ZONE_STYLE[k].color : "#5b86b3", borderStyle: ZONE_STYLE[k].dashArray ? "dashed" : "solid" }} />{l}
                </button>))}
            </span>
          )}
          <button data-testid="map-toggle-satellite-layer" onClick={() => setShowSat(!showSat)} title="NASA GIBS VIIRS true colour on acquisition date" className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider" style={{ ...overlayBtn, color: showSat ? "#0a67ad" : "#5b86b3" }}><Globe2 size={12} /> Satellite</button>
          ...
          <button data-testid="btn-focus-spill" onClick={focusSpill} className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider" style={{ ...overlayBtn, color: focus ? "#b26a00" : "#0a2540" }}><Crosshair size={12} /> Focus spill</button>
          ...
          <span className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 font-mono text-[11px] uppercase tracking-wider" style={{ ...overlayBtn, color: liveAis?.state === "LIVE" ? "#006a61" : "#5b86b3" }} data-testid="map-live-vessels-chip" title="Live AISStream vessels within 250 km of the slick (green dots)">● live AIS {liveAis?.state || "…"} · {nearLive.length} near slick</span>
          {overlayMeta?.has_quicklook && (
            <span className="inline-flex items-center gap-2 rounded px-2.5 py-1.5" style={overlayBtn} data-testid="scene-overlay-control">
              <button data-testid="map-toggle-scene-overlay" onClick={() => setShowOverlay(!showOverlay)} className="inline-flex items-center gap-1 font-mono text-[11px] uppercase tracking-wider" style={{ color: showOverlay ? "#0a67ad" : "#5b86b3" }}><ImageIcon size={12} /> SAR quicklook</button>
        ...
        <div className="absolute bottom-3 left-3 right-3 z-[1000] flex items-end gap-3">
          <div className="rounded p-3 text-[11px] shrink-0" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid var(--border-default)", backdropFilter: "blur(12px)" }} data-testid="map-legend">
            ...
            <div className="flex items-center gap-2 mt-1"><span className="h-2.5 w-4 border border-dashed" style={{ borderColor: "#0a67ad" }} /> Search corridor</div>
            ...
            <div className="flex items-center gap-2 mt-1"><span className="h-0 w-4 border-t-2 border-dashed" style={{ borderColor: "#5b86b3" }} /> Interpolated AIS gap (dead reckoning)</div>
