...
const FOOT_STYLE = { color: "#D4604D", weight: 1.5, fillColor: "#D4604D", fillOpacity: 0.35 };
const ORIGIN_STYLE = { color: "#F7F6F2", fillColor: "#D4604D", fillOpacity: 1 };
      ...
      <div className="grid flex-1 gap-0 overflow-hidden lg:grid-cols-[1fr_420px]">
        <div className="relative flex flex-col">
          ...
          <div className="flex flex-wrap items-center gap-3 border-t px-4 py-2" style={{ borderColor: "var(--border-default)", background: "var(--bg-secondary)" }} data-testid="vault-timeline">
            <button data-testid="vault-play" onClick={() => setPlaying(!playing)} className="rounded border p-1.5 text-slate-700" style={{ borderColor: "var(--border-highlight)" }}>{playing ? <Pause size={12} /> : <Play size={12} />}</button>
        ...
        <aside className="overflow-y-auto border-l p-4 text-xs" style={{ borderColor: "var(--border-default)", background: "var(--bg-secondary)" }} data-testid="vault-evidence-panel">
          <h2 className="mb-2 flex items-center gap-2 font-display text-sm font-semibold"><Scale size={13} color="#C48A22" /> Legal evidence</h2>
          {ev ? (
            <dl className="space-y-2">
              {[["Ruling", ev.ruling], ["Penalties", ev.penalties], ["Cleanup cost", ev.cleanup_cost], ["Compensation", ev.compensation]].map(([k, val]) => <div key={k}><dt className="label-mono">{k}</dt><dd className="text-slate-700">{val}</dd></div>)}
            </dl>
          ) : <p className="text-slate-500" data-testid="vault-no-legal">No structured legal record for this entry yet.</p>}
          <h2 className="mb-2 mt-5 flex items-center gap-2 font-display text-sm font-semibold"><Leaf size={13} color="#2E8B6A" /> Ecological evidence</h2>
          <p className="text-slate-700">{ev?.ecological || e.ecosystems?.join(", ")}</p>
          ...
          <p className="mt-2 italic text-tide/80">{e.lessons}</p>
          {ev?.sources?.length > 0 && (
            <div className="mt-4" data-testid="vault-sources"><p className="label-mono mb-1">Source documents</p>
              {ev.sources.map((s) => <a key={s.u} href={s.u} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-tide hover:underline"><ExternalLink size={10} /> {s.t}</a>)}
            </div>
          )}
        </aside>
      </div>
