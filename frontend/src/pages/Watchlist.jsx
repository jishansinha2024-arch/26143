      ...
      <div className="grid gap-4 xl:grid-cols-[360px_1fr]">
        {sup ? (
          <div className="panel p-5 fade-up" data-testid="watchlist-form">
            <div className="mb-4 flex items-center gap-2"><Eye size={16} color="#D4604D" /><h2 className="font-display text-lg font-semibold">Flag a vessel</h2></div>
              ...
              <tbody>
                {active.map((w) => (
                  <tr key={w.id} data-testid={`watchlist-row-${w.mmsi}`} className="border-t" style={{ borderColor: "var(--border-default)" }}>
                    ...
                    <td className="px-4 py-2.5 text-right">{sup && <button data-testid={`watchlist-remove-${w.mmsi}`} onClick={() => remove(w)} className="rounded p-1 text-slate-400 hover:text-rose-600"><Trash2 size={13} /></button>}</td>
                  </tr>
                ))}
