...
const Preview = ({ s }) => {
  ...
  if (failed) return <div ref={ref} className="grid h-24 w-32 shrink-0 place-items-center rounded bg-mist text-[10px] text-slate-500">no preview</div>;
  return url ? <img src={url} alt={s.stac_id} loading="lazy" className="h-24 w-32 shrink-0 rounded object-cover" data-testid={`scene-preview-${s.stac_id}`} /> : <div ref={ref} className="grid h-24 w-32 shrink-0 place-items-center rounded bg-mist"><ImageIcon size={16} color="#506672" className="animate-pulse" /></div>;
};
    ...
    <div className="flex h-full overflow-hidden" data-testid="explorer-page">
      <div className="relative flex-1">
        <MapContainer center={[40, 10]} zoom={3} className="h-full w-full" worldCopyJump doubleClickZoom={!drawing}>
          ...
          {asset?.geometry && <GeoJSON key={`asset-${asset.id}`} data={asset.geometry} style={{ color: "#C48A22", weight: 2, dashArray: "8,4", fillColor: "#C48A22", fillOpacity: 0.06 }} onEachFeature={(ft, l) => l.bindTooltip(`${asset.name} · ${asset.type}`, { permanent: true, direction: "top" })} />}
          <ViewTracker onView={setView} onZoom={setZoom} /><FlyTo bbox={flyTo} />
          {showZones && <ZonesLayer types={["eez", "territorial", "contiguous"]} />}
          {aoi?.geometry && <GeoJSON key={`aoi-${aoi.updated_at}`} data={aoi.geometry} style={{ color: "#2A93A8", weight: 2, dashArray: "10,5", fillOpacity: 0.04 }} onEachFeature={(ft, l) => l.bindTooltip(`AOI · ${aoi.name} (${aoi.provenance})`, { sticky: true })} />}
          ...
          {footprints && <GeoJSON key={res.scenes.map((s) => s.stac_id).join("|") + hover} data={footprints}
            style={(ft) => ({ color: ft.properties.id === hover ? "#C48A22" : "#2A93A8", weight: ft.properties.id === hover ? 2.5 : 1, fillOpacity: ft.properties.id === hover ? 0.2 : 0.05 })}
        ...
        <div className="absolute left-3 top-3 z-[1000] flex flex-wrap items-center gap-2">
          ...
          <button data-testid="explorer-toggle-zones" onClick={() => setShowZones(!showZones)} className="rounded px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-wider" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid var(--border-highlight)", color: showZones ? "#1F7F93" : "#7D919C" }}>EEZ zones</button>
          {PRESETS.map(([l, b]) => <button key={l} data-testid={`preset-${l.replace(/[^a-z]/gi, "").toLowerCase()}`} onClick={() => setFlyTo(b)} className="rounded px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-wider text-slate-700" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid var(--border-highlight)", backdropFilter: "blur(12px)" }}><MapPin size={10} className="mr-1 inline" />{l}</button>)}
        </div>
        ...
        <div className="rounded p-2.5 text-[11px]" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid var(--border-default)", backdropFilter: "blur(12px)" }} data-testid="gibs-control">
          ...
          <div className="flex items-center gap-2">
            <select data-testid="gibs-layer-select" value={basemap} onChange={(e) => setBasemap(e.target.value)} className="rounded border bg-mist px-2 py-1 font-mono text-[10px] text-slate-700 outline-none" style={bd}>
              <option value="">off</option>{meta?.basemaps.map((b) => <option key={b.id} value={b.id}>{b.label}</option>)}
            </select>
            <input data-testid="gibs-date-input" type="date" value={gibsDate} max={iso(new Date())} onChange={(e) => setGibsDate(e.target.value)} className="rounded border bg-mist px-2 py-1 font-mono text-[10px] text-slate-700 outline-none" style={bd} />
          </div>
        </div>
        <div className="rounded p-2.5 text-[11px]" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid var(--border-default)", backdropFilter: "blur(12px)" }} data-testid="density-control">
          ...
          <select data-testid="density-window-select" value={densityHours} onChange={(e) => setDensityHours(+e.target.value)} className="rounded border bg-mist px-2 py-1 font-mono text-[10px] text-slate-700 outline-none" style={bd}>
          ...
          <div className="mt-1 font-mono text-[10px]" data-testid="live-vessels-summary" style={{ color: liveAis?.state === "LIVE" ? "#2E8B6A" : "#7D919C" }}>● live AIS ({liveAis?.source || "AISStream"}) · {liveAis?.state || "…"} · {liveAis?.count ?? 0} vessel(s) on map</div>
        </div>
        </div>
      </div>
      <aside className="flex w-[480px] shrink-0 flex-col overflow-hidden border-l" style={{ borderColor: "var(--border-default)", background: "var(--bg-secondary)" }}>
        <div className="border-b p-4" style={{ borderColor: "var(--border-default)" }}>
          ...
          <div className="mt-3 flex items-center gap-2">
            <button data-testid="btn-search-scenes" disabled={busy || !bbox} onClick={search} className="inline-flex items-center gap-1.5 rounded bg-ink px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-paper hover:bg-tide disabled:opacity-50"><Search size={12} /> {busy ? "Searching…" : "Search current map view"}</button>
            ...
            {res?.scenes.map((s) => (
              <div key={s.stac_id} data-testid={`scene-result-${s.stac_id}`} onMouseEnter={() => setHover(s.stac_id)} onMouseLeave={() => setHover(null)} className="flex gap-3 rounded border p-2.5 text-xs transition-colors hover:border-amber-400/50" style={{ borderColor: "var(--border-default)" }}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5"><Satellite size={11} color="#2A93A8" /><span className="font-display font-semibold">{s.platform?.toUpperCase()}</span><span className="font-mono text-[10px] text-slate-400">{fmtTime(s.datetime)}</span></div>
                  <div className="truncate font-mono text-[10px] text-tide" title={s.stac_id}>{s.stac_id}</div>
