...
export const SceneWatches = ({ bbox, collection }) => {
  ...
  return (
    <div className="border-t p-4" style={{ borderColor: "var(--border-default)" }} data-testid="scene-watches">
      <div className="mb-1 flex items-center gap-2"><Radar size={13} color="#C48A22" /><span className="font-display text-sm font-semibold">Auto Scene Watch</span><span className="ml-auto font-mono text-[10px] text-slate-500">cron every 3 h</span></div>
        ...
        {rows.map((w) => (
          <div key={w.id} data-testid={`scene-watch-${w.id}`} className="rounded border p-2 text-[11px]" style={{ borderColor: "var(--border-default)", opacity: w.active ? 1 : 0.5 }}>
            <div className="flex items-center gap-2"><span className="font-display font-semibold">{w.name}</span><span className="font-mono text-[10px] text-slate-500">{w.collection}{w.auto_detect ? " · auto-detect" : ""}</span>
              {sup && <span className="ml-auto flex gap-1"><button data-testid={`scene-watch-run-${w.id}`} disabled={busy} onClick={() => run(w)} title="Poll now" className="rounded p-1 text-slate-400 hover:text-tide disabled:opacity-40"><Play size={11} /></button><button data-testid={`scene-watch-toggle-${w.id}`} onClick={() => toggle(w)} className="rounded px-1.5 font-mono text-[10px] text-slate-400 hover:text-amber-700">{w.active ? "pause" : "resume"}</button><button data-testid={`scene-watch-delete-${w.id}`} onClick={() => remove(w)} className="rounded p-1 text-slate-400 hover:text-rose-600"><Trash2 size={11} /></button></span>}</div>
      ...
      {sup && (
        <div className="mt-2 flex items-center gap-2">
          ...
          <label className="flex shrink-0 items-center gap-1 font-mono text-[10px] text-slate-600"><input type="checkbox" data-testid="scene-watch-auto-toggle" checked={auto} onChange={(e) => setAuto(e.target.checked)} /> detect</label>
          <button data-testid="btn-create-scene-watch" disabled={busy || name.trim().length < 2 || !bbox} onClick={create} className="shrink-0 rounded px-3 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-slate-950 disabled:opacity-50" style={{ background: "#C48A22" }}>Watch this view</button>
        </div>
      )}
    </div>
  );
