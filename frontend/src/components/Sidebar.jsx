...
const itemBase = "group relative flex items-center rounded-xl border py-2.5 font-label-lg text-label-lg outline-none transition-all focus-visible:ring-2 focus-visible:ring-primary/50";
const itemState = (isActive) => (isActive ? "border-primary/25 bg-primary-fixed text-primary font-semibold shadow-[0_1px_6px_rgba(20,121,196,0.12)]" : "border-transparent text-on-surface-variant hover:bg-surface-container hover:text-on-surface");
...
const SectionLabel = ({ children }) => <p className="px-space-md pb-space-xs pt-space-sm font-mono text-[10px] font-medium uppercase tracking-[0.2em] text-slate-500">{children}</p>;
...
const NavBody = ({ collapsed, onNavigate }) => {
  ...
  return (
    <nav className="flex flex-1 flex-col gap-space-md overflow-y-auto overflow-x-hidden px-space-sm py-space-md [&::-webkit-scrollbar]:w-1.5" data-testid="sidebar-nav">
      ...
      <div className="mt-auto flex flex-col gap-1 border-t border-[var(--border-default)] pt-space-md">
        {refPinned && (
          <div title={collapsed ? `Reference case pinned: ${ref?.case_number || ""}` : undefined}
            className={`group relative mb-space-xs flex items-center rounded-xl border border-[var(--border-default)] bg-surface-container-low py-space-sm ${collapsed ? "justify-center px-0" : "gap-space-md px-space-md"}`} data-testid="sidebar-reference-status">
...
export const Sidebar = ({ collapsed, onToggleCollapse, mobileOpen, onCloseMobile }) => (
  <>
    {/* Desktop */}
    <aside className={`hidden shrink-0 flex-col bg-surface-container-lowest border-r border-[var(--border-default)] shadow-[1px_0_10px_rgba(10,79,148,0.05)] md:flex ${collapsed ? "w-[68px]" : "w-64"} z-30 transition-[width] duration-200`} data-testid="sidebar-desktop">
      ...
      <button data-testid="sidebar-collapse-toggle" onClick={onToggleCollapse} title={collapsed ? "Expand sidebar" : "Collapse sidebar"}
        className="flex items-center justify-center gap-2 border-t border-[var(--border-default)] py-2.5 text-on-surface-variant transition-colors hover:bg-surface-container hover:text-on-surface">
        <Icon name={collapsed ? "keyboard_double_arrow_right" : "keyboard_double_arrow_left"} className="!text-[18px]" />
        {!collapsed && <span className="font-mono text-[10px] font-medium uppercase tracking-[0.18em]">Collapse sidebar</span>}
      </button>
    </aside>
    ...
    {mobileOpen && (
      <div className="fixed inset-0 z-50 md:hidden" data-testid="sidebar-mobile-overlay">
        ...
        <aside className="absolute left-0 top-0 flex h-full w-64 flex-col bg-surface-container-lowest shadow-2xl" data-testid="sidebar-mobile">
          <div className="flex h-16 shrink-0 items-center justify-between border-b border-[var(--border-default)] px-space-lg">
            <span className="flex items-center gap-space-sm font-display text-lg font-semibold text-on-surface"><span className="grid h-9 w-9 place-items-center rounded-xl border border-primary/20 bg-primary-fixed text-primary"><span className="material-symbols-outlined text-[20px]">radar</span></span> Varuna <span className="text-blue-grad">Netra</span></span>
