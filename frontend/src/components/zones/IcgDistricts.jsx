...
const REGION_COLOR = { NW: "#C48A22", W: "#D9762E", E: "#1F7F93", NE: "#8B6BCB", AN: "#2E8B6A" };
export const icgColor = (code) => REGION_COLOR[code] || "#7D919C";
      ...
      <div className="grid gap-1.5 md:grid-cols-2" data-testid="icg-district-list">
        {data.districts.map((d) => (
          <div key={d.code} data-testid={`icg-district-${d.code}`} className="flex items-center gap-2 rounded border px-2.5 py-1.5 text-xs" style={{ borderColor: "var(--border-default)", opacity: d.active ? 1 : 0.5 }}>
            ...
            {admin && <button data-testid={`icg-edit-${d.code}`} onClick={() => { setEdit({ ...d }); setGeom(""); }} className="font-mono text-[10px] text-tide hover:underline">edit</button>}
          </div>
        ))}
      </div>
      {edit && (
        <div className="mt-3 grid grid-cols-2 gap-2 rounded border p-3" style={bd} data-testid="icg-edit-form">
          ...
          <label className="flex items-center gap-2 font-mono text-[11px] text-slate-600"><input data-testid="icg-edit-active" type="checkbox" checked={edit.active} onChange={(e) => setEdit({ ...edit, active: e.target.checked })} /> active</label>
