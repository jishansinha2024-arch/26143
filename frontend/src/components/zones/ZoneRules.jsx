...
const SEV = { high: "#D4604D", medium: "#C48A22", low: "#7D919C" };
...
export const ZoneRules = ({ zones }) => {
  ...
  return (
    <div className="mt-5 rounded border p-4" style={{ borderColor: "rgba(217,118,46,0.4)", background: "rgba(217,118,46,0.04)" }} data-testid="zone-rules">
      <div className="mb-2 flex items-center gap-2"><BellRing size={14} color="#D9762E" /><h2 className="font-display font-semibold">Zone alert rules</h2><span className="ml-auto font-mono text-[10px] text-slate-500">{activeRules} active</span></div>
        ...
        {rules.map((r) => (
          <div key={r.id} data-testid={`zone-rule-${r.id}`} className="rounded border p-2.5 text-xs" style={{ borderColor: "var(--border-default)", opacity: r.active ? 1 : 0.5 }}>
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-tide">{r.zone_code}</span>
              {sup && <span className="ml-auto flex items-center gap-1">
                <button data-testid={`zone-rule-toggle-${r.id}`} onClick={() => toggle(r)} className="rounded px-1.5 py-0.5 font-mono text-[10px] text-slate-400 hover:text-amber-700">{r.active ? "disable" : "enable"}</button>
                <button data-testid={`zone-rule-delete-${r.id}`} onClick={() => remove(r)} className="rounded p-1 text-slate-400 hover:text-rose-600"><Trash2 size={12} /></button>
              </span>}
            </div>
      ...
      {sup && (
        <div className="mt-3 grid grid-cols-2 gap-2" data-testid="zone-rule-form">
          ...
          <label className="flex items-center gap-2 font-mono text-[11px] text-slate-600"><input data-testid="rule-primary-only-toggle" type="checkbox" checked={f.primary_only} onChange={(e) => setF({ ...f, primary_only: e.target.checked })} /> primary jurisdiction only</label>
