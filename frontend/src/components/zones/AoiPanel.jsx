...
export const ProvBadge = ({ p, testId }) => <span data-testid={testId} className="rounded px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider" style={{ color: PROV_COLOR[p] || "#7D919C", border: `1px solid ${PROV_COLOR[p] || "#7D919C"}55` }}>{p || "UNVERIFIED"}</span>;
...
export const AoiPanel = ({ onFlyTo, onAoiChange, drawing, setDrawing, drawnGeometry, onDrawnConsumed, compact = false }) => {
  ...
  return (
    <div className="rounded border p-3" style={{ borderColor: "rgba(31,127,147,0.35)", background: "rgba(31,127,147,0.04)" }} data-testid="aoi-panel">
      <div className="mb-2 flex items-center gap-2"><Globe2 size={14} color="#2A93A8" /><h2 className="font-display font-semibold">Investigation AOI · worldwide</h2>
        {aoi && analyst && <button data-testid="aoi-clear" onClick={clear} className="ml-auto rounded p-1 text-slate-400 hover:text-rose-600" title="clear AOI"><X size={12} /></button>}</div>
      {aoi ? (
        <div className="mb-2 rounded border p-2 text-[11px]" style={{ borderColor: "var(--border-default)" }} data-testid="aoi-current">
          <div className="flex items-center gap-2"><Crosshair size={11} color="#2A93A8" /><span className="text-slate-800">{aoi.name}</span><ProvBadge p={aoi.provenance} testId="aoi-provenance" /></div>
          ...
          <button data-testid="aoi-fly" onClick={() => onFlyTo?.(aoi.bbox)} className="mt-1 font-mono text-[10px] uppercase tracking-wider text-tide hover:underline">fly to AOI</button>
        </div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5" data-testid="aoi-presets">
        {Object.entries(presets).map(([k, p]) => (
          <button key={k} data-testid={`aoi-preset-${k}`} disabled={busy || !analyst} onClick={() => select({ kind: "preset", preset: k })} className="rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider disabled:opacity-40" style={{ borderColor: k === "sih_mumbai" ? "rgba(184,134,42,0.6)" : "var(--border-highlight)", color: k === "sih_mumbai" ? "#C48A22" : "#CFCBC0" }} title={p.note || p.name}>{k === "sih_mumbai" ? "★ SIH · Mumbai" : p.name}</button>))}
        {analyst && setDrawing && <button data-testid="aoi-draw-toggle" onClick={() => setDrawing(!drawing)} className="inline-flex items-center gap-1 rounded border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ borderColor: "rgba(124,92,191,0.6)", color: drawing ? "#F7F6F2" : "#8B6BCB", background: drawing ? "rgba(124,92,191,0.6)" : "transparent" }}><PenTool size={10} /> {drawing ? "drawing… (Esc)" : "draw custom AOI"}</button>}
      </div>
