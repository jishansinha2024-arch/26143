...
export const EmailSettings = ({ onChanged }) => {
  ...
  return (
    <div className="panel p-5 fade-up" data-testid="email-settings">
      <div className="mb-3 flex items-center gap-2"><Mail size={16} color="#2A93A8" /><h2 className="font-display text-lg font-semibold">Email delivery (Resend)</h2>
        <span data-testid="email-configured-badge" className="ml-auto rounded-full px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: s.configured ? "#2E8B6A" : "#C48A22", border: `1px solid ${s.configured ? "#2E8B6A" : "#C48A22"}66` }}>{s.configured ? "configured" : "not configured"}</span></div>
      ...
      {s.last_test && <p className="mt-2 font-mono text-[10px]" style={{ color: s.last_test.sent ? "#2E8B6A" : "#D4604D" }} data-testid="email-last-test">last test {fmtTime(s.last_test.at)} → {s.last_test.sent ? `sent (${s.last_test.id})` : `failed: ${s.last_test.error}`}</p>}
    </div>
  );
};
