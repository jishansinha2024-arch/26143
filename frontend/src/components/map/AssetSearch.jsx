...
const TYPE_COLOR = { oil_field: "#C48A22", basin: "#D9762E", terminal: "#1F7F93", shipping_lane: "#2E8B6A", country: "#7D919C", eez: "#2A93A8", other: "#CFCBC0" };
...
export const AssetSearch = ({ onSelect, compact }) => {
  ...
  return (
    <div className={`relative ${compact ? "w-72" : "w-96"}`} data-testid="asset-search">
      <div className="flex items-center gap-2 rounded border bg-mist px-2.5 py-1.5" style={{ borderColor: "var(--border-highlight)", backdropFilter: "blur(12px)" }}>
        <Search size={12} color="#7D919C" />
        <input data-testid="asset-search-input" value={q} onChange={(e) => setQ(e.target.value)} onFocus={() => hits.length && setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 150)} onKeyDown={(e) => e.key === "Enter" && hits[0] && pick(hits[0])}
          placeholder="Search country, EEZ, oil field… e.g. Bombay High, KG Basin" className="w-full bg-transparent font-mono text-xs text-slate-800 outline-none placeholder:text-slate-500" />
      </div>
      {open && hits.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-[1200] mt-1 max-h-80 overflow-y-auto rounded border" style={{ background: "rgba(255,255,255,0.96)", borderColor: "var(--border-highlight)", backdropFilter: "blur(12px)" }} data-testid="asset-search-results">
          {hits.map((h) => (
            <button key={h.id} data-testid={`asset-hit-${h.id}`} onMouseDown={() => pick(h)} className="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-xs hover:bg-paper/70">
              <MapPin size={11} color={TYPE_COLOR[h.type] || "#CFCBC0"} />
              <span className="truncate text-slate-800">{h.name}</span>
              <span className="ml-auto shrink-0 font-mono text-[10px] uppercase tracking-wider" style={{ color: TYPE_COLOR[h.type] || "#CFCBC0" }}>{h.type}</span>
