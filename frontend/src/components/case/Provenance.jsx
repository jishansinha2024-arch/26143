...
const BADGE = { "REAL SENTINEL-1": "text-emerald-700 border-emerald-400/60", LIVE: "text-emerald-700 border-emerald-400/60", HISTORICAL: "text-tide border-tide/60", REFERENCE: "text-tide border-sky-400/60", EXPERIMENTAL: "text-amber-700 border-amber-400/60", MOCK: "text-rose-600 border-rose-400/60", DEMO: "text-rose-600 border-rose-400/60", UNAVAILABLE: "text-slate-400 border-slate-500", NONE: "text-slate-400 border-slate-500", ANALYST: "text-tide border-tide/60", EXTERNAL: "text-slate-600 border-slate-500", "USER-DEFINED": "text-violet-300 border-violet-400/60" };
...
const R = ({ k, v, testid }) => <div className="flex justify-between gap-2 border-b border-dashed py-0.5" style={{ borderColor: "var(--border-default)" }}><span className="text-slate-500">{k}</span><span className="text-right text-slate-700 break-all" data-testid={testid}>{v ?? "UNAVAILABLE"}</span></div>;
...
export const Provenance = ({ caseId }) => {
  ...
  return (
    <div className="mx-4 my-3 rounded border p-3 font-mono text-[11px]" style={{ borderColor: "var(--border-highlight)", background: "rgba(31,127,147,0.03)" }} data-testid="provenance-panel">
      <div className="mb-2 flex flex-wrap items-center gap-2"><Fingerprint size={13} className="text-tide" /><span className="font-display text-sm font-semibold">Data provenance</span><Badge v={p.data_mode} testid="provenance-data-mode" />{p.case_origin_label && <Badge v={p.case_origin_label} testid="provenance-case-origin" />}</div>
      <div className="grid gap-x-6 gap-y-1 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div><p className="label-mono mb-1 flex items-center gap-2">Detection <Badge v={p.detection.badge} testid="provenance-det-badge" /></p>
          ...
          <p className="mt-1 text-[10px] text-amber-700/80" data-testid="provenance-validation-note">{p.detection.validation}</p>
        </div>
      ...
      {(p.ais.badge === "NONE" || p.ais.badge === "UNAVAILABLE") && <p className="mt-2 text-amber-700/90" data-testid="provenance-ais-none">No AIS observations for this window — satellite analysis is valid, vessel attribution is unavailable for this case.</p>}
    </div>
  );
};
