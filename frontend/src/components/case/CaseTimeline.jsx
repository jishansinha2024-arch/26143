    ...
    <div className="p-4" data-testid="case-timeline">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {hasRole(user, "supervisor") && (
          <>
            <select data-testid="share-expiry-select" value={hours} onChange={(e) => setHours(e.target.value)} className="rounded border bg-mist px-2 py-1.5 font-mono text-[11px] text-slate-700 outline-none" style={{ borderColor: "var(--border-highlight)" }}>
              {[[24, "24 h"], [72, "3 days"], [168, "7 days"], [720, "30 days"]].map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
            <input data-testid="share-note-input" value={note} onChange={(e) => setNote(e.target.value)} placeholder="recipient / agency (optional)" className="w-44 rounded border bg-mist px-2 py-1.5 font-mono text-[11px] text-slate-700 outline-none" style={{ borderColor: "var(--border-highlight)" }} />
            <button data-testid="btn-create-share" disabled={busy} onClick={share} className={btn} style={{ borderColor: "rgba(184,134,42,0.5)", color: "#C48A22" }}><Share2 size={12} /> {busy ? "Creating…" : "Create share link"}</button>
          </>
        )}
      </div>
          ...
          {shares.map((s) => {
            ...
            return (
              <div key={s.id} data-testid={`share-link-${s.id}`} className="flex items-center gap-2 rounded border px-2.5 py-1.5 font-mono text-[10px]" style={{ borderColor: "var(--border-default)", opacity: dead ? 0.5 : 1 }}>
                <Link2 size={11} color={dead ? "#5F7684" : "#C48A22"} />
                <span className="text-slate-600">{s.recipient_note || "share link"}</span>
                <span className="text-slate-500">by {s.created_by} · {s.revoked ? "revoked" : `expires ${fmtTime(s.expires_at)}`} · {s.views} views</span>
                {!dead && hasRole(user, "supervisor") && <button data-testid={`revoke-share-${s.id}`} onClick={() => revoke(s)} className="ml-auto inline-flex items-center gap-1 text-rose-600 hover:underline"><Ban size={10} /> revoke</button>}
              </div>
            );
          })}
        </div>
      ...
      <ol className="relative ml-2 mt-4 space-y-3 border-l pl-5" style={{ borderColor: "var(--border-highlight)" }} data-testid="timeline-events">
        {tl.events.map((e, i) => (
          <li key={`${e.at || e.t || ""}-${e.kind}-${i}`} className="relative text-xs" data-testid={`timeline-event-${e.kind}`}>
            <span className="absolute -left-[26px] top-1 h-2.5 w-2.5 rounded-full" style={{ background: KIND_COLOR[e.kind] || "#7D919C", boxShadow: "0 0 0 3px var(--bg-secondary)" }} />
            <div className="flex flex-wrap items-baseline gap-x-2">
              ...
              <span className="font-display font-semibold text-slate-800">{e.title}</span>
              {e.actor && <span className="font-mono text-[10px] text-tide">{e.actor}{e.role ? ` · ${e.role}` : ""}</span>}
            </div>
