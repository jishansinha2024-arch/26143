...
const STATE_UI = {
  SAR_READY: { color: "#2E8B6A", label: "SENTINEL-1 SAR READY", hint: "Real Sentinel-1 SAR attached · dark-vessel scan available" },
  QUICKLOOK_GENERATING: { color: "#C48A22", label: "QUICKLOOK GENERATING", hint: "SAR available · preview still being generated" },
  NO_SCENE_SELECTED: { color: "#7D919C", label: "NO SCENE SELECTED", hint: "Run the acquisition search to find the nearest real Sentinel-1 pass" },
  SEARCHING: { color: "#2A93A8", label: "SEARCHING SENTINEL-1", hint: "Querying Planetary Computer STAC (±36 h → ±72 h → ±5 d → ±7 d)" },
  NO_36H: { color: "#C48A22", label: "NO ACQUISITION IN ±36 H", hint: "" },
  NEAREST_FOUND_EXTENDED: { color: "#C48A22", label: "NEAREST SENTINEL-1 FOUND (EXTENDED WINDOW)", hint: "" },
  NO_COVERAGE_7D: { color: "#D4604D", label: "NO SENTINEL-1 ACQUISITION FOUND", hint: "Satellite revisit gap for this AOI/time. AIS/jurisdiction investigation can continue." },
  STAC_FAILED: { color: "#D4604D", label: "STAC SEARCH FAILED", hint: "" },
  SAR_ASSET_UNAVAILABLE: { color: "#D4604D", label: "SAR ASSET UNAVAILABLE", hint: "Scene attached but no Sentinel-1 analysis asset" },
  ATTACH_FAILED: { color: "#D4604D", label: "SCENE ATTACHMENT FAILED", hint: "" },
};
  ...
  return (
    <div className="mt-2 space-y-1.5 font-mono text-[10px]" data-testid="scene-sar-status">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {s.sar_confirmation === "PENDING" && !skipSar && <span className="rounded px-1.5 py-0.5 uppercase tracking-wider text-amber-700" style={{ border: "1px solid rgba(184,134,42,0.4)" }} data-testid="sar-confirmation-pending">SAR confirmation pending</span>}
      ...
      {s.scene_id && s.acquisition_time && (
        <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-slate-600 md:grid-cols-4" data-testid="scene-acquisition-info">
          <span>Target event: <b className="text-slate-800">{fmtTime(acquisitionTime)}</b></span><span>Acquired: <b className="text-slate-800">{fmtTime(s.acquisition_time)}</b></span>
          <span>Difference: <b className="text-slate-800">{fmtDiff(s.time_difference_hours)}</b></span><span>AOI overlap: <b className="text-slate-800">{s.overlap_percent != null ? `${s.overlap_percent}%` : "—"}</b>{s.search_window_hours ? ` · window ±${s.search_window_hours} h` : ""}{s.polarization ? ` · ${s.polarization}` : ""}</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1.5">
        {!s.scene_id && <button data-testid="btn-attach-scene" disabled={!!busy} onClick={() => attach()} className="inline-flex items-center gap-1 rounded border px-2 py-0.5 uppercase tracking-wider text-tide hover:bg-tide/10 disabled:opacity-50" style={{ borderColor: "rgba(31,127,147,0.4)" }}><Satellite size={10} /> {busy === "auto" ? "Searching STAC…" : state === "NO_SCENE_SELECTED" ? "Find nearest Sentinel-1 scene" : "Retry search"}</button>}
        <button data-testid="btn-view-scenes" disabled={!!busy} onClick={view} className="rounded border px-2 py-0.5 uppercase tracking-wider text-slate-600 hover:text-ink disabled:opacity-50" style={{ borderColor: "var(--border-highlight)" }}>{busy === "list" ? "Searching…" : s.scene_id ? "Change scene" : "View available scenes"}</button>
        {!s.scene_id && !skipSar && <button data-testid="btn-continue-without-sar" onClick={() => setSkipSar(true)} className="rounded border px-2 py-0.5 uppercase tracking-wider text-slate-400 hover:text-ink" style={{ borderColor: "var(--border-default)" }}>Continue without SAR</button>}
        {skipSar && <span className="text-amber-700" data-testid="without-sar-note">Continuing with AIS + jurisdiction only · SAR confirmation pending · re-check satellite later</span>}
      </div>
      {cands && (
        <div className="rounded border p-2" style={{ borderColor: "var(--border-highlight)" }} data-testid="scene-candidates">
          <div className="mb-1 flex flex-wrap items-center gap-2 text-slate-600"><b className="text-slate-800">AVAILABLE SENTINEL-1 SCENES</b> · PRIMARY: Sentinel-1 SAR · {cands.found ? `${cands.candidates.length} in ±${cands.search_window_hours} h` : cands.reason}</div>
          {cands.candidates.map((c) => (
            <div key={c.scene_id} className="flex flex-wrap items-center gap-2 border-t py-1" style={{ borderColor: "var(--border-default)" }} data-testid={`scene-candidate-${c.scene_id}`}>
              <span className="text-tide">{c.scene_id}</span><span className="text-slate-400">{fmtTime(c.acquisition_time)}</span><span>{c.signed_offset_hours > 0 ? "+" : "−"}{fmtDiff(c.time_difference_hours)}</span>
              ...
              {c.scene_id === s.provider_scene_id ? <span className="ml-auto text-emerald-700">attached</span> : <button data-testid={`btn-use-scene-${c.scene_id}`} disabled={!!busy} onClick={() => attach(c.scene_id)} className="ml-auto rounded bg-ink px-1.5 py-0.5 font-semibold uppercase text-paper disabled:opacity-50">{busy === c.scene_id ? "…" : "Use scene"}</button>}
            </div>))}
...
export const DarkVessels = ({ caseId, onScan, sceneStatus, acquisitionTime, onAttached }) => {
  ...
  return (
    <div className="mx-4 my-3 rounded border p-3" style={{ borderColor: "rgba(194,90,73,0.45)", background: "rgba(194,90,73,0.04)" }} data-testid="dark-vessels-panel">
      <div className="flex flex-wrap items-center gap-2">
        <Radar size={13} color="#D4604D" /><span className="font-display text-sm font-semibold">Dark vessel scan</span>
        <span className="rounded px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider text-amber-700" style={{ border: "1px solid currentColor" }}>experimental · CFAR</span>
        <span className="ml-auto flex items-center gap-2 font-mono text-[11px]">
          <label className="text-slate-400">radius <input data-testid="dark-scan-radius" type="number" min={5} max={150} value={radius} onChange={(e) => setRadius(+e.target.value)} className="w-14 rounded border bg-mist px-1 py-0.5 text-slate-800" style={{ borderColor: "var(--border-highlight)" }} /> km</label>
      ...
      {error && <p className="mt-2 text-[11px] text-rose-600" data-testid="dark-scan-error">{error}</p>}
      ...
      {scan?.targets && scan.status !== "not_scanned" && (
        <>
          <p className="mt-2 text-[11px] text-slate-400" data-testid="dark-summary">{scan.bright_targets_total} bright targets in {scan.analysis_input?.kind === "sar_aoi_window" ? `real SAR AOI window (${scan.analysis_input.asset})` : "scene quicklook"} · {scan.targets.length} within {scan.radius_km} km · <span className="text-rose-600">{scan.dark_count} without AIS ≤ {scan.dark_radius_km} km (±{scan.time_window_min} min, {scan.ais_fixes_checked} fixes checked)</span> · {fmtTime(scan.created_at)}</p>
          {noAis && <p className="mt-1 text-[10px] text-amber-700" data-testid="dark-no-ais-warning">No AIS fixes exist for this time window — every bright target is unmatched by construction. Connect live AIS or ingest historical AIS before treating any target as "dark".</p>}
          <p className="mt-1 text-[10px] text-amber-700/80">{scan.disclaimer}</p>
          {dark.length > 0 && (
            <table className="mt-2 w-full text-[11px]" data-testid="dark-vessels-table">
              ...
              <tbody>{dark.map((t, i) => (
                <tr key={t.id} data-testid={`dark-vessel-${t.id}`} className="border-t" style={{ borderColor: "var(--border-default)" }}>
                  <td className="px-2 py-1 font-mono text-rose-600">D{i + 1}</td>
                  <td className="px-2 py-1 font-mono text-slate-700">{t.lat.toFixed(4)}, {t.lon.toFixed(4)}</td>
