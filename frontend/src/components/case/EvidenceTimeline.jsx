...
const ICON = { "case.opened": "#2A93A8", "case.correlated": "#1F7F93", "review.confirm": "#2E8B6A", "review.reject": "#D4604D", "review.needs_more_data": "#C48A22", "alert.raised": "#D4604D", "spill.created": "#D4604D", "scene.registered": "#8B6BCB" };
  ...
  return (
    <div className="space-y-5 p-4 text-sm" data-testid="evidence-panel">
      <section>
        ...
        <div className="rounded border p-3 font-mono text-[11px] leading-relaxed text-slate-600" style={{ borderColor: "var(--border-default)", background: "var(--bg-secondary)" }} data-testid="evidence-sources">
          ...
          {calculations && <div>Algorithm: {calculations.algorithm_version} · input hash <span className="text-tide">{calculations.input_hash.slice(0, 16)}…</span>{calculations.degraded && <span className="text-purple-300"> · DEGRADED (no drift inputs)</span>}</div>}
        </div>
      </section>
      <section>
        ...
        <div className="space-y-1">
          {result_versions.map((v) => (
            <div key={v.version} className="flex items-center gap-3 font-mono text-[11px]" data-testid={`result-version-${v.version}`}>
              <span className="rounded bg-paper px-1.5 py-0.5 text-tide">v{v.version}</span>
          ...
          {reviews.map((r) => (
            <div key={r.id} className="rounded border p-2.5 text-xs" style={{ borderColor: "var(--border-default)" }} data-testid={`review-${r.id}`}>
              <div className="flex items-center gap-2 font-mono text-[11px]">
                ...
                {r.vessel_mmsi && <span className="text-slate-600">MMSI {r.vessel_mmsi}</span>}
              </div>
              {r.reason_codes.length > 0 && <div className="mt-1 font-mono text-[10px] text-tide">{r.reason_codes.join(" · ")}</div>}
        ...
        <ol className="relative ml-2 space-y-2 border-l pl-4" style={{ borderColor: "var(--border-highlight)" }} data-testid="audit-timeline">
          {[...audit_history].reverse().map((e) => (
            <li key={e.id} className="relative text-xs">
              <span className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full border-2" style={{ borderColor: ICON[e.action] || "#5F7684", background: "var(--bg-card)" }} />
              <div className="font-mono text-[11px]"><span style={{ color: ICON[e.action] || "#7D919C" }}>{e.action}</span> <span className="text-slate-500">· {e.actor} · {fmtTime(e.created_at)}</span></div>
